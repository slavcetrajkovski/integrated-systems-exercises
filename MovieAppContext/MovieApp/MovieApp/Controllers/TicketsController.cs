...
namespace MovieApp.Controllers
{
    public class TicketsController : Controller
    {
        ...
        public async Task<IActionResult> AddTicketToCart(Guid? id)
        {
            ...
            var movie = await _context.Movies.FirstOrDefaultAsync(m => m.Id == ticket.MovieId);

            if (ticket != null)
            {
                var model = new AddTicketToCartDTO
                {
                    SelectedMovie = movie?.MovieDescription,
                    TicketId = ticket.Id,
                    Quantity = 1
                };
                return View(model);
